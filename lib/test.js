...
Test.prototype.expect = function(a, b, c){
  var self = this;
  this.snapshot = new ErrorWithCaller(Test.prototype.expect);
  // callback
...
Test.prototype.assert = function(res, fn){
  ...
  for (var i = 0; i < bodies.length; i++) {
    ...
    if ('object' == typeof body && !isregexp) {
      try {
        ...
        return fn(this.makeError('expected ' + a + ' response body, got ' + b, body, res.body));
      }
    } else {
      // string
      if (body !== res.text) {
        ...
        if (isregexp) {
          if (!body.test(res.text)) {
            return fn(this.makeError('expected body ' + b + ' to match ' + body, body, res.body));
          }
        } else {
          return fn(this.makeError('expected ' + a + ' response body, got ' + b, body, res.body));
        }
  ...
  for (var field in fields) {
    ...
    if (null == actual) return fn(this.makeError('expected "' + field + '" header field'));
    for (var i = 0; i < expecteds.length; i++) {
      ...
      if (re) return fn(this.makeError('expected "' + field + '" matching ' + fieldExpected + ', got "' + actual + '"'));
      return fn(this.makeError('expected "' + field + '" of "' + fieldExpected + '", got "' + actual + '"'));
    }
  }
  ...
  if (status && res.status !== status) {
    ...
    return fn(this.makeError('expected ' + status + ' "' + a + '", got ' + res.status + ' "' + b + '"'), res);
  }
  ...
  for (var i = 0; i < this._asserts.length; i++) {
    ...
    if (!err) continue;
    return fn(err instanceof Error ? err : this.makeError(err))
  }
...
Test.prototype.makeError = function makeError(msg, expected, actual) {
  var err = this.snapshot;
  err.message = msg;
...
util.inherits(ErrorWithCaller, Error)
